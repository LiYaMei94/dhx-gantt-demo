  ...
  function add_export_methods(gantt) {
    ...
    function get_styles(css) {
      ...
      return (color_hash[css] =
        get_color(color_box, "color") +
        ";" +
        get_color(color_box, "backgroundColor"));
    }
      ...
      return function (code, parent, master) {
        ...
        if (branch)
          for (var i = 0; i < branch.length; i++) {
            ...
            if (
              (!start || item.end_date > start) &&
              (!end || item.start_date < end)
            )
    ...
    gantt._send_to_export = function (data, type) {
      var convert = this.date.date_to_str(
        this.config.date_format || this.config.xml_date
      );
      if (data.config) {
    ...
    function copy_object_table(obj) {
      ...
      copy.$left = tmps.leftside_text
        ? tmps.leftside_text(obj.start, obj.end_date, obj)
        : "";
    ...
    function fix_columns(gantt, columns) {
      for (var i = 0; i < columns.length; i++) {
        columns[i].label =
          columns[i].label || gantt.locale.labels["column_" + columns[i].name];
    ...
    gantt._serialize_plain = function () {
      var config = gantt.templates.xml_format || gantt.templates.format_date;
      gantt.templates.xml_format = gantt.date.date_to_str(
        "%Y%m%dT%H%i%s",
        true
      );
    ...
    gantt._serialize_table = function (config) {
      gantt.json._copyObject = config.visual
        ? copy_object_colors
        : copy_object_table;
      var data = export_serialize();
      gantt.json._copyObject = original;
        ...
        if (css) {
          ...
          for (var i = 0; i < data.data.length; i++) {
            ...
            for (var j = 0; j < steps.length; j++) {
              ...
              if (cell_css)
                data.data[i].styles.push({
                  index: j,
                  styles: get_styles(cell_css),
                });
            }
          }
        }
    ...
    gantt._serialize_scales = function (config) {
      ...
      for (var i = 0; i < raw.length; i++) {
        ...
        var template =
          ...
          step.template ||
          (step.date
            ? gantt.date.date_to_str(step.date)
    ...
    gantt._serialize_columns = function (config) {
      ...
      for (var i = 0; i < cols.length; i++) {
        ...
        columns[ccount] = {
          ...
          header:
            cols[i].label || gantt.locale.labels["column_" + cols[i].name],
...
(function () {
  ...
  function clear_circ_dependencies(
    task,
    links,
    tasks,
    usedTasks,
  ...
  function exportConfig(data, config) {
    ...
    var manual =
      config.auto_scheduling === undefined ? false : !!config.auto_scheduling;
  ...
  function add_export_methods(gantt) {
    ...
    gantt.exportToMSProject = function (config) {
      ...
      config.skip_circular_links =
        config.skip_circular_links === undefined
          ? true
          : !!config.skip_circular_links;
    ...
    function sendImportAjax(config) {
      ...
      if (config.taskProperties)
        settings.taskProperties = config.taskProperties;
